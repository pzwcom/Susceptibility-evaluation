...
class Machine_Learning:
    def RandomForest(train_sample:Literal,trainsize:float,modeldir:Str,param_grid=None):
        ...
        X=train_sample[0][:,2:]
        y=train_sample[1]
        X_train,X_val,y_train,y_val= train_test_split(X,y,train_size=trainsize,random_state=42)
        random_model=RandomForestClassifier(random_state=100)
        print(np.mean(cross_val_score(random_model, X, y, cv=5, scoring='roc_auc')))
        #格网法确定超参数
        if(param_grid!=None):
        ...
        pre_val_zheng=clt.predict(X_val)
        pre_val=clt.predict_proba(X_val)
        # 计算因子重要性
        # importances = clt.feature_importances_
        # for f in range(X_train.shape[1]):
        #     print(importances[f])
        # 展示roc
        cm = confusion_matrix(y_val,pre_val_zheng,normalize='true')
        disp = ConfusionMatrixDisplay(confusion_matrix=cm)
        disp.plot()
        plt.show()
        display_roc(y_val,pre_val,'randforest_roc')
        del pre_val
    ...
    def SVM(train_sample:Literal,trainsize:float,modeldir:Str,param_grid=None):
            ...
            #格网法确定超参数
            if(param_grid!=None):
                grid = GridSearchCV(random_model,param_grid,cv = 5,scoring = 'roc_auc',n_jobs = -1)
                #在训练集上训练
                grid.fit(X_train,y_train)
                #返回最优的训练器
                random_model = grid.best_estimator_
